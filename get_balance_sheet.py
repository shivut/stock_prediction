...
def get_balance_sheet(sectorwise_com, company_code_mapping, path, logger):
    ...
    for each_sector, each_sector_companies in sectorwise_com.items():
        ...
        if each_sector_companies:
            if not os.path.exists(os.path.join(path, "balance_sheet", each_sector)):
                os.makedirs(os.path.join(path, "balance_sheet", each_sector))
            ...
            for each_com in each_sector_companies:
                ...
                for each_division in range(1, 6):
                    ...
                    each_division_table_data = get_financial_data(each_com.replace(" ", ""), company_code_mapping.get(each_com), "balance-sheet", each_division)

                    if each_division_table_data:
                        csv_writer(os.path.join(path, "balance_sheet", each_sector, each_com + "_" + str(each_division) + ".csv"), each_division_table_data)
                    else:
                        break
...
if __name__ == '__main__':
    ...
    if mapping_path:
        ...
        is_success = get_balance_sheet(mapping_obj["sectorwise_com"], mapping_obj["company_code_mapping"], mapping_path, logger_obj)
