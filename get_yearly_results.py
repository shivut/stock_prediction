...
def get_profit_loss_statement(sectorwise_com, company_code_mapping, path, logger):
    ...
    for each_sector, each_sector_companies in sectorwise_com.items():
        ...
        if each_sector_companies:
            if not os.path.exists(os.path.join(path, "yearly_results", each_sector)):
                os.makedirs(os.path.join(path, "yearly_results", each_sector))

            for each_com in each_sector_companies:
                logger.debug("getting the yearly results statement for " + each_com)
